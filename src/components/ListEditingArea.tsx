import React, { useEffect, useState, useRef, SyntheticEvent, useReducer } from "react";
...
const DEFAULT_COMPLETE_LIST: completeListType = {
	id: 0,
	title: "",
	likersCount: 0,
...
type updateListActionType = {
	type: "updateList";
	updateProps: putListType;
};

type setList = {
	type: "setList";
	list: completeListType;
};
...
export const ListEditingArea = () => {
	...
	const listReducer = (previousList: completeListType, action: listActionType) => {
		switch (action.type) {
			case "deleteItem":
				return {
					...previousList,
					items: previousList.items.filter((item) => item.id !== action.id),
				};
			case "updateItem":
				if (action.idType === "id") {
					return {
						...previousList,
						items: previousList.items.map((existingItem) =>
							existingItem.id === action.id ? action.item : existingItem
						),
					};
				} else {
					return {
						...previousList,
						items: previousList.items.map((existingItem) =>
							existingItem.externalApiIdentifier === String(action.id) ? action.item : existingItem
						),
					};
				}
			case "addItem":
				return { ...previousList, items: [...previousList.items, action.item] };
			case "updateList":
				return { ...previousList, ...action.updateProps };
			case "setList":
				return { ...action.list };
			case "switchItems":
				return {
					...
					items: previousList.items.map((item, index) => {
						if (index === action.sourceIndex) {
							return { ...previousList.items[action.destinationIndex], rank: index + 1 };
						} else if (index === action.destinationIndex)
							return { ...previousList.items[action.sourceIndex], rank: index + 1 };
						else return item;
					}),
				};
			case "updateItemsRank":
				return {
					...previousList,
					items: previousList.items
						.filter((item) => item.id !== action.removeItemId)
						.map((item, index) => (index > action.removeItemIndex - 1 ? { ...item, rank: item.rank - 1 } : item)),
				};
			default:
	...
	const [list, dispatchList] = useReducer(listReducer, DEFAULT_COMPLETE_LIST);
	...
	useEffect(() => {
		if (id === undefined) navigate("/manage-lists");
			...
			getSingleList(+id)
				.then((response) => {
					dispatchList({ type: "setList", list: response.data });
	...
	const listItemTypeFormatter = (item: simplifiedListItemType): simplifiedListItemType => {
		return {
			id: item.id,
		...
		if (itemBeingEdited === null) {
			insertItem(Number(list?.id), newItem)
				.then((response) => {
					...
					dispatchList({ type: "addItem", item: addedItem });
			...
			updateItem(newItem, itemBeingEdited.id)
				.then((response) => {
					...
					dispatchList({ type: "updateItem", id: replacedItem.id, idType: "id", item: replacedItem });
			...
			deleteItem(removeItemId)
				.then((_response) => {
					...
					if (success) {
						...
						dispatchList({ type: "updateItemsRank", removeItemId, removeItemIndex });
					}
				})
				.catch((_error) =>
	...
	const handleSaveItemEditedOnClick = (_e: React.MouseEvent<HTMLButtonElement, MouseEvent>) => {
		updateItem(itemBeingEdited!, itemBeingEdited!.id)
			.then((response) => {
				...
				dispatchList({
					type: "updateItem",
					id: Number(itemBeingEdited!.externalApiIdentifier),
					idType: "apiId",
					item: editedItem,
				});
				setItemBeingEdited(null);
	...
	const handlePublishListOnChange = (_e: SyntheticEvent<Element, Event>, checked: boolean) => {
		if (list!.items.length < 3 && checked) {
			...
			updateList(list!.id, { draft: false })
				.then((_response) => {
					dispatchList({ type: "updateList", updateProps: { draft: true } });
		...
		updateItem({ rank: destinationIndex + 1 }, list!.items[sourceIndex].id)
			.then(() => {
				updateItem({ rank: sourceIndex + 1 }, list!.items[destinationIndex].id)
					.then(() => {
						console.log("Work in progress");
						dispatchList({ type: "switchItems", sourceIndex, destinationIndex });
					})
					.catch((_error) =>
...
// Component only With useState (no useReducer)
// export const ListEditingArea = () => {
// 	const { id } = useParams();
// 	const navigate = useNavigate();
